...
`app/` に Kotlin + Jetpack Compose で実装した Android アプリ版を追加しています。端末の NFC-F でカードを読み取り、デスクトップ版と同じ項目を表示します。

- カード識別情報（IDm / PMm / IDi / PMi）と発行者
- 発行情報（取り込み済みフラグを含む）
- 残高と属性フラグ（音声案内サービス、定期有効期間外のSF利用、タッチでGo！新幹線）
- 取引履歴（最大 20 件、残高差分つき）
- 定期券情報（区間・券番・発売額・購入時支払方法・R通番）
- オートチャージの契約状況・しきい値・チャージ額
- 改札入出場情報、SF改札入場情報、料金発券情報
- カード情報全体の JSON コピー／共有

認証サーバが関与するのは相互認証までです。サーバからセッション情報を受け取った後は
アプリがカードから直接ブロックを読み出すため、カードの中身がネットワークに出ることはありません。

### 認証サーバの設定

既定の認証サーバは `https://felica-auth.nyaa.ws` です。アプリバーの **設定** から
自前のサーバなど別の URL に変更できます。設定は端末に保存され、空欄で保存すると
既定値に戻ります。
...
- 概要タブの先頭に残高を大きく表示するヒーローカード、続けて主要項目を一覧表示
- 右上のボタンでライト／ダークテーマを切り替え
- 履歴タブでは取引履歴をテーブル表示。1 取引ごとの増減額（差額）列を備え、チャージは緑で強調。列見出しクリックで並び替え、入力ボックスで全文検索フィルタが可能 (`Ctrl+F` / `Cmd+F` でフォーカス)
- 改札タブで改札履歴・装置番号・金額・定期区間を表示（列見出しで並び替え可）、SF 改札入場情報と、搭載カードでは料金発券・改札情報も併記
- データタブでカード情報 JSON を閲覧し、クリップボードへコピー／ファイル出力、取引履歴の CSV 書き出しにも対応
...
- カードを自動検出し、リロードなしでページをライブ更新します。デスクトップ GUI と同じ体験（残高ヒーローカード、タブ構成、差額付きの並び替え可能な取引履歴、改札テーブル、JSON/CSV 出力、ライト／ダークテーマ）。
...
Web GUI はソースまたはインストール済み wheel（`uv run` / `pip install`）から動作します。ビルド済み単一実行ファイルは CLI とデスクトップ GUI のみが対象です。
...
`v*` タグを push すると [`.github/workflows/android-release.yml`](.github/workflows/android-release.yml) が Android APK をビルドし、GitHub Release に `suica-viewer-android-<tag>.apk` として添付します。
